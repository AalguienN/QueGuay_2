...
public class FXMLInicioController implements Initializable {
    ...
    @FXML
    private Label ContraseñaIncorrecta2;
    @FXML
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        contraseña.focusedProperty().addListener((observable,oldValue,newValue) -> {
            if(!newValue){
                checkEditPassword();
            }
        });
        ...
        Aceptar.disableProperty().bind(Bindings.not(validFields));
    ...
    private void checkEditPassword(){
        if(!Utils.checkPassword(contraseña.textProperty().getValueSafe()))
            manageError(ContraseñaIncorrecta,contraseña,validPassword);
        else
            manageCorrect(ContraseñaIncorrecta,contraseña,validPassword);
    }
    private void checkEquals(){
        if(contraseña.textProperty().getValueSafe().compareTo(
            contraseña2.textProperty().getValueSafe()) != EQUALS){
                showErrorMessage(ContraseñaIncorrecta2,contraseña2);
                equalPasswords.setValue(Boolean.FALSE);
                contraseña.textProperty().setValue("");
                contraseña2.textProperty().setValue("");
                contraseña.requestFocus();
        }else
            manageCorrect(ContraseñaIncorrecta2,contraseña2,equalPasswords);
        
    }

    @FXML
    private void AceptarActivo(ActionEvent event) {
